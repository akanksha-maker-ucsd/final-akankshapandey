...
@app.get('/mvp_updates', response_class=HTMLResponse)
def get_home(request:Request) -> HTMLResponse:
  session = sessions.get_session(request)
  if len(session) > 0 and session.get('logged_in'):
    session_id = request.cookies.get("session_id")
    template_data = {'request':request, 'session':session, 'session_id':session_id}
    return views.TemplateResponse('mvp_updates.html', template_data)
  else:
    return RedirectResponse(url="/login", status_code=302)
...
def get_users() -> dict:
  ...
  keys = ['id', 'first_name', 'last_name', 'username', 'email', 'student_id']
...
def post_user(user:User) -> dict:
  new_id = db.create_user(user.first_name, user.last_name, user.username, user.password, user.email, user.student_id)
...
def put_user(user_id:int, user:User) -> dict:
  print(user);
  return {'success': db.update_user(user_id, user.first_name, user.last_name, user.username, user.password, user.email, user.student_id)}
